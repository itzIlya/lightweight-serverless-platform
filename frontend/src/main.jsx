...
function Header({ user, screen, onHome, onWorkspace, onGuide, onLogin, onLogout }) {
  return <header className="topbar"><button className="wordmark" onClick={onHome}>serverless<span>.</span></button><nav><button onClick={onWorkspace}>Workspace</button><button onClick={onGuide}>How it works</button></nav><div className="account">{user ? <><span>{user.username}</span><button className="button quiet" onClick={onLogout}>Log out</button></> : <button className="button" onClick={onLogin}>{screen === "auth" ? "Sign in" : "Get started"}</button>}</div></header>;
}
function Landing({ onStart, onGuide }) {
  return <main className="landing"><section className="hero"><p className="eyebrow">A small serverless control plane</p><h1>Turn a Python function into a live endpoint.</h1><p className="lede">Write code, declare its contract, build it, and run it. The platform keeps the machinery in the background and shows you the next useful thing.</p><div className="hero-actions"><button className="button primary" onClick={onStart}>Start building <span>→</span></button><button className="button quiet" onClick={onGuide}>Learn the platform</button></div></section><section id="guide" className="flow-strip"><FlowCard number="01" title="Shape it" text="Describe the function, paste code, and define files."/><FlowCard number="02" title="Build it" text="Install dependencies and prepare a callable version."/><FlowCard number="03" title="Run it" text="Send JSON or files, follow the result, download artifacts."/></section></main>;
}
